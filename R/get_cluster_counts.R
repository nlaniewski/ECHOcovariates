get_cluster_counts<-function(cluster.counts.long.filepath,cluster.annotations.filepath=NULL){
  ...
  cluster.counts[,data.table::`:=`(batch = factor(sprintf("%03d",batch)))]
  cluster.counts[,data.table::`:=`(cluster = factor(cluster,levels=sort(unique(cluster))))]
  ##relevel visit
  cluster.counts[,data.table::`:=`(visit = factor(visit,levels=c(paste0("V",c(4,6,7)),"Adult"),
                                      labels = c("Birth","6-months","12-months","Adult")))]
  ##
  ...
  if(!is.null(cluster.annotations.filepath)){
    ...
    cluster.annotations[,data.table::`:=`(cluster = factor(cluster,levels=sort(unique(cluster))))]
    cluster.annotations[,data.table::`:=`(cell.type = factor(cell.type,levels=as.character(levels(cluster.counts$cell.type))))]
    ##merge
